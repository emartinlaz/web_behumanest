...
@rx.page(route=Route.CONSENTIMIENTOS.value, title="Consentimientos")
def consentimientos() -> rx.Component:
    return rx.hstack(
        menu_lateral(),
        rx.vstack(
            cabecera(titulo="Consentimientos", mostrar_link_area_privada=True),
            rx.vstack(
                rx.card(
                    rx.vstack(
                        rx.image(
                            "/consentimientos_1.png",
                        ),
                    align="center",
                    padding="0px", 
                    width="100%",
